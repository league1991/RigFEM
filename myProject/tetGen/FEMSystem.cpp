...
bool RigFEM::RiggedSkinMesh::setWeight( const char* weightFile )
{
	...
	Utilities::eigDense2Sparse(matMap["weight"], m_weightMat);
	if (m_weightMat.rows() != m_intDofIdx.size() ||
		m_weightMat.cols() != m_surfDofIdx.size())
		return false;
...
bool RigFEM::RiggedSkinMesh::computeHessian( const EigVec&x, const EigVec& param, EigDense& H )
{
	...
	*m_tangentStiffnessMatrix *= -1;
	...
	EigDense dbp = (dFss + m_weightMatTran * (dFns + dFnn*m_weightMat) + dFsn*m_weightMat) * J;
...
{
	EigVec s;
	bool res = computeSurfOffset(&p[0], m_t, s);
	EigVec n = m_weightMat * s;
